...
- **Web dashboard redesign** (`burndown serve`): now wears the landing page's dark,
  ...
  self-contained (no external fonts or scripts) and 127.0.0.1-only.
- **Terminal readout reformatted** to match the landing's hero mock: a compact
  metadata line, an annotated two-line programmatic / interactive split, and a
  highlighted runway row (a green left bar). And `budget` / `scope` / `currency` now
  print a one-line confirmation instead of re-dumping the full readout, so chaining
  set-commands no longer floods the screen.
